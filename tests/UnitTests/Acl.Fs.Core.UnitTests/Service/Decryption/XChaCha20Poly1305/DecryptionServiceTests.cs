...
using Acl.Fs.Core.Service.Decryption.XChaCha20Poly1305;
using Microsoft.Extensions.Logging;
using Moq;

namespace Acl.Fs.Core.UnitTests.Service.Decryption.XChaCha20Poly1305;
...
public sealed class DecryptionServiceTests : IDisposable
{
    ...
    public DecryptionServiceTests()
    {
        ...
        _tempDirectory = Path.Combine(Path.GetTempPath(), "AclFsTests", "XChaCha20Poly1305DecryptionService",
    ...
    public async Task DecryptFileAsync_WithValidParameters_CallsDecryptorBaseWithCorrectParameters()
    {
        ...
        var input = new XChaCha20Poly1305DecryptionInput(decryptionKey);
        ...
        _mockDecryptorBase.Verify(x => x.ExecuteDecryptionProcessAsync(
            ...
            It.Is<byte[]>(key => key.Length == 32 && key.SequenceEqual(decryptionKey)),
            _mockLogger.Object,
    ...
    public async Task DecryptFileAsync_PassesCorrectKeyToDecryptorBase()
    {
        ...
        var input = new XChaCha20Poly1305DecryptionInput(expectedKey);
    ...
    public async Task DecryptFileAsync_WithMultipleDecryptions_HandlesEachIndependently()
    {
        ...
        var input1 = new XChaCha20Poly1305DecryptionInput(decryptionKey1);
        var input2 = new XChaCha20Poly1305DecryptionInput(decryptionKey2);
        ...
        Assert.True(decryptionKey1.SequenceEqual(capturedKeys[0]), "First decryption should use the correct key");
        Assert.True(decryptionKey2.SequenceEqual(capturedKeys[1]), "Second decryption should use the correct key");
