...
namespace Acl.Fs.Core.UnitTests.Services.Encryption.Shared.Buffer;
...
public sealed class BufferManagerTests
{
    ...
    {
        using var bufferManager = new BufferManager(0);
    ...
    {
        const int metadataBufferSize = 1024;
        var bufferManager = new BufferManager(metadataBufferSize);
    ...
    public void UsingStatement_ShouldAutomaticallyCallDispose()
    {
        ...
        var exception = Record.Exception(() =>
        {
            using (bufferManager = new BufferManager(metadataBufferSize))
            {
