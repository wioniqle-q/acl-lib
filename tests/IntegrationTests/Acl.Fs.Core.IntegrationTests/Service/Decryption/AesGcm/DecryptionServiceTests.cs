...
namespace Acl.Fs.Core.IntegrationTests.Service.Decryption.AesGcm;
...
public sealed class DecryptionServiceTests : IDisposable
{
    ...
    public DecryptionServiceTests()
    {
        ...
        services.AddAesGcmDecryptionServices();
    ...
    public async Task DecryptFileAsync_WithValidEncryptedFile_ShouldDecryptSuccessfully()
    {
        const string originalContent =
            "This is a test file content for encryption and decryption testing with AES-GCM.";
    ...
    [Fact]
    public async Task DecryptFileAsync_WithAes128Key_ShouldDecryptSuccessfully()
    {
        const string originalContent = "Testing AES-128 encryption and decryption with 16-byte key.";
        var sourcePath = Path.Combine(_testDataDirectory, "aes128-original-file.txt");
        var encryptedPath = Path.Combine(_testOutputDirectory, "aes128-encrypted-file.enc");
        var decryptedPath = Path.Combine(_testDecryptedDirectory, "aes128-decrypted-file.txt");
        ...
        Assert.True(File.Exists(decryptedPath), "Decrypted AES-128 file should exist");
    ...
    [Fact]
    public async Task DecryptFileAsync_WithAes192Key_ShouldDecryptSuccessfully()
    {
        const string originalContent = "Testing AES-192 encryption and decryption with 24-byte key.";
        var sourcePath = Path.Combine(_testDataDirectory, "aes192-original-file.txt");
        var encryptedPath = Path.Combine(_testOutputDirectory, "aes192-encrypted-file.enc");
        var decryptedPath = Path.Combine(_testDecryptedDirectory, "aes192-decrypted-file.txt");
        ...
        Assert.True(File.Exists(decryptedPath), "Decrypted AES-192 file should exist");
    ...
    [Fact]
    public async Task DecryptFileAsync_WithAes256Key_ShouldDecryptSuccessfully()
    {
        const string originalContent = "Testing AES-256 encryption and decryption with 32-byte key.";
        var sourcePath = Path.Combine(_testDataDirectory, "aes256-original-file.txt");
        var encryptedPath = Path.Combine(_testOutputDirectory, "aes256-encrypted-file.enc");
        var decryptedPath = Path.Combine(_testDecryptedDirectory, "aes256-decrypted-file.txt");
        ...
        Assert.True(File.Exists(decryptedPath), "Decrypted AES-256 file should exist");
    ...
    public async Task DecryptFileAsync_WithWrongKey_ShouldThrowException()
    {
        ...
        var encryptInstruction = new FileTransferInstruction(sourcePath, encryptedPath);
        var encryptionInput = new AesEncryptionInput(encryptionKey);
        ...
        var decryptInstruction = new FileTransferInstruction(encryptedPath, decryptedPath);
        var decryptionInput = new AesDecryptionInput(decryptionKey);
    ...
    [Fact]
    public async Task DecryptFileAsync_WithDifferentKeySizes_ShouldThrowException()
    {
        const string originalContent = "Testing different key sizes between encryption and decryption.";
        var sourcePath = Path.Combine(_testDataDirectory, "keysize-mismatch-original-file.txt");
        var encryptedPath = Path.Combine(_testOutputDirectory, "keysize-mismatch-encrypted-file.enc");
        var decryptedPath = Path.Combine(_testDecryptedDirectory, "keysize-mismatch-decrypted-file.txt");
    ...
    public async Task DecryptFileAsync_WithInvalidEncryptedFile_ShouldThrowException()
    {
        ...
        await File.WriteAllTextAsync(invalidEncryptedPath, "This is not a valid AES-GCM encrypted file");
    ...
    {
        var testCases = new List<(string source, string encrypted, string decrypted, string content, int keySize)>();

        var keySizes = new[] { 16, 24, 32 };

        foreach (var keySize in keySizes)
        {
            var sourcePath = Path.Combine(_testDataDirectory, $"sequential-original-{keySize * 8}.txt");
            var encryptedPath = Path.Combine(_testOutputDirectory, $"sequential-encrypted-{keySize * 8}.enc");
            var decryptedPath = Path.Combine(_testDecryptedDirectory, $"sequential-decrypted-{keySize * 8}.txt");
            var content =
                $"Sequential test content for AES-{keySize * 8} - this content should be preserved through encryption and decryption.";

            await File.WriteAllTextAsync(sourcePath, content);
            testCases.Add((sourcePath, encryptedPath, decryptedPath, content, keySize));
        }

        using var cancellationTokenSource = new CancellationTokenSource(TimeSpan.FromMinutes(3));

        foreach (var (source, encrypted, _, _, keySize) in testCases)
        {
            ...
            await File.WriteAllBytesAsync(encrypted + ".key", key, cancellationTokenSource.Token);
        }

        foreach (var (_, encrypted, decrypted, _, _) in testCases)
        {
            var key = await File.ReadAllBytesAsync(encrypted + ".key", cancellationTokenSource.Token);
        ...
        foreach (var (_, encrypted, decrypted, originalContent, _) in testCases)
        {
