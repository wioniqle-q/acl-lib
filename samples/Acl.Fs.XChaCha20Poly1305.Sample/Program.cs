...
namespace Acl.Fs.XChaCha20Poly1305.Sample;
...
internal static class Program
{
    ...
    private static byte[] GenerateSecureXChaCha20Key()
    {
    ...
    private static string GenerateXChaCha20MasterKey()
    {
        var key = GenerateSecureXChaCha20Key();
        try
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var encryptedFilePath = Path.Combine(directory, $"xchacha20_encrypted_{fileName}{fileExtension}");
            var decryptedFilePath = Path.Combine(directory, $"xchacha20_decrypted_{fileName}{fileExtension}");

            var fileId = Guid.NewGuid().ToString();
            var encryptInstruction = new FileTransferInstruction(sourceFilePath, encryptedFilePath);

            Console.WriteLine(
                $"[{Thread.CurrentThread.ManagedThreadId}] Processing {fileInfo.Name} ({fileInfo.Length:N0} bytes) with XChaCha20Poly1305...");
            ...
            try
            {
                ...
                Console.WriteLine(
                    $"[{Environment.CurrentManagedThreadId}] XChaCha20Poly1305 encrypted {fileInfo.Name} in {encryptionStopwatch.ElapsedMilliseconds}ms");
                ...
                try
                {
                    ...
                    var decryptionInput = new XChaCha20Poly1305DecryptionInput(retrievedKey);
                    ...
                    Console.WriteLine(
                        $"[{Environment.CurrentManagedThreadId}] XChaCha20Poly1305 decrypted {fileInfo.Name} in {decryptionStopwatch.ElapsedMilliseconds}ms");
                    return true;
                }
                finally
                {
    ...
    private static async Task Main()
    {
        ...
        var serviceProvider = new ServiceCollection()
            ...
            .AddXChaCha20Poly1305Factory()
            .AddXChaCha20Poly1305EncryptionServices()
            .AddXChaCha20Poly1305DecryptionServices()
            .AddAuditLogger()
        ...
        var masterPublicKey = GenerateXChaCha20MasterKey();
        ...
        try
        {
            Console.WriteLine($"Processing {sourceFilePaths.Length} files with XChaCha20Poly1305 (max concurrency: {MaxConcurrency})");
            ...
            Console.WriteLine($"\nCompleted XChaCha20Poly1305 processing of {totalCount} files in {overallStopwatch.ElapsedMilliseconds}ms");
        ...
        Console.WriteLine("\nXChaCha20Poly1305 processing completed. Press any key to exit.");
        Console.ReadKey();
